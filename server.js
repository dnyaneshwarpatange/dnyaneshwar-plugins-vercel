...
app.use(express.static(path.join(__dirname, 'public')));  // local fallback
app.use(session({
...
// ─── Serve Frontend ───────────────────────────────────────────────────────────
app.get('*', (req, res) => {
  // Try both paths — works locally and on Vercel
  const fromRoot    = path.join(__dirname, '..', 'public', 'index.html');
  const fromCurrent = path.join(__dirname, 'public', 'index.html');
  const fs = require('fs');
  if (fs.existsSync(fromRoot)) {
    res.sendFile(fromRoot);
  } else {
    res.sendFile(fromCurrent);
  }
});
